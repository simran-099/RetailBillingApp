...
#,bg="#03C04A"
    ...
    def __init__(self,root):
        ...
        self.root.title("SnackBazaar")
        bg_color="#0BB5FF"
        title=Label(self.root,text="SnackBazaar",bd=10,relief=GROOVE,bg=bg_color,fg="#003151",font=("times new roman",30,"bold"),pady=2).pack(fill=X)
        #=====var====
        #====juices======
        self.sprite=IntVar()
       ...
       # Email of Customer
        c_email_lbl = Label(F1, text="Email",  bg=bg_color, fg="#003151",font=("times new roman", 15, "bold"))
        c_email_lbl.grid(row=0, column=6, padx=20, pady=5)

        self.c_email = StringVar()
        c_email_txt = Entry(F1, width=20, textvariable=self.c_email, font="arial 10", bd=7, relief=SUNKEN)
        c_email_txt.grid(row=0, column=7, pady=10, padx=5)
        
       #search button for searching bill 
        csv_btn = Button(F1, text="Search", command=self.search_bill_in_csv, bg="#03C04A", fg="white", width=12, bd=5, font=("times new roman",15,"bold"))
        csv_btn.grid(row=0, column=8, padx=5, pady=10)
       ...
       #======Juices Frame=======
        ...
        nicker_lbl1=Label(F4,text="Snickers",font=("times new roman",16,"bold"),bg=bg_color,fg="#003151").grid(row=3,column=0,padx=10,pady=10,sticky="w")
        ...
        btn_A.place(x=750,width=710,height=105)

        total = Button(btn_A, command=self.total, text="Total", bg="#03C04A", fg="white",bd=5, pady=15, width=11, font="arial 13 bold")
        total.grid(row=0, column=0, padx=5, pady=5)

        generate_bill = Button(btn_A, command=self.bill_area, text="Generate Bill", bg="#03C04A", fg="white", bd=5, pady=15, width=11, font="arial 13 bold")
        generate_bill.grid(row=0, column=1, padx=5, pady=5)

        email_btn = Button(btn_A, text="Email PDF", font="arial 13 bold", bg="#03C04A", fg='white',bd=5, pady=15, width=11, command=self.send_pdf_bill)
        email_btn.grid(row=0, column=2, padx=5, pady=5)

        clear = Button(btn_A, text="Clear", command=self.clear_data, bg="#03C04A", fg="white",bd=5, pady=15, width=11, font="arial 13 bold")
        clear.grid(row=0, column=3, padx=5, pady=5)

        Exit_Btn = Button(btn_A, text="Exit", command=self.Exit_app, bg="#03C04A", fg="white",bd=5, pady=15, width=11, font="arial 13 bold")
    ...
    def save_bill(self):
        ...
        if op > 0:
           ...
           folder_path = os.path.join(os.getcwd(), "bills")
           os.makedirs(folder_path, exist_ok=True)

           bill_path = os.path.join(folder_path, f"{self.bill.get()}.txt")
           with open(bill_path, "w") as f1:
                f1.write(self.bill_data)

           csv_file = os.path.join(os.getcwd(), "sales_data.csv")
           sales_data = {
            "Date": [datetime.now().strftime("%Y-%m-%d %H:%M:%S")],
            "Bill No": [self.bill.get()],
            "Customer Name": [self.c_name.get()],
            "Phone": [self.c_phon.get()],
            "Juice Total": [self.total_juice_price],
            "Chips Total": [self.total_chips_price],
            "Chocolate Total": [self.total_chocolate_price],
    ...
    def find_bill(self):
        ...
        present = "no"
        bills_folder = os.path.join(os.getcwd(), "bills")  # ✅ Use dynamic path

        if not os.path.exists(bills_folder):
           messagebox.showerror("Error", "Bills folder not found.")
           return

        for i in os.listdir(bills_folder):
            if i.split('.')[0] == self.search_bill.get():
               with open(os.path.join(bills_folder, i), "r") as f1:
                   self.txtarea.delete('1.0', END)
                   for d in f1:
                    self.txtarea.insert(END, d)
            present = "yes"
            break  # bill mil gaya toh break
    ...
    def send_pdf_bill(self):
       if not self.bill_data:
          messagebox.showerror("Error", "No bill data to send.")
          return

       customer_email = self.c_email.get()  # Optional: You can add an Email field in GUI

       filename = f"bill_{self.bill.get()}.pdf"
       filepath = os.path.join(os.getcwd(), filename)
       c = canvas.Canvas(filepath)
       textobject = c.beginText(40, 800)

       for line in self.bill_data.split('\n'):
       ...
       try:
         msg = EmailMessage()
         msg['Subject'] = f"Your Bill No. {self.bill.get()}"
         msg['From'] = EMAIL  # Your Gmail
         msg['To'] = customer_email           # Customer's Email
         msg.set_content("Please find attached your bill (PDF).")
         ...
         with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
            smtp.login(EMAIL, APP_PASSWORD)  # App password
            smtp.send_message(msg)

         messagebox.showinfo("Success", "PDF Bill sent successfully!")

       except Exception as e:
            messagebox.showerror("Error", f"Email send failed:\n{str(e)}")
  
  
  
  
    def search_bill_in_csv(self):
        import pandas as pd
        ...
        try:
           ...
           result = df[df["Bill No"] == int(bill_no)]

           if result.empty:
            messagebox.showerror("Not Found", f"No bill found in CSV with Bill No: {bill_no}")
           else:
            top = Toplevel(self.root)
            top.title(f"Bill Details: {bill_no}")
            top.geometry("600x250")
            row = result.iloc[0]

            y = 10
            for col, val in row.items():
                Label(top, text=f"{col}: {val}", font=("Arial", 12)).place(x=20, y=y)
                y += 30
...
def save_bill(self):
    ...
    if op > 0:
        ...
        # Save bill as text file in "bills" folder next to the .py file
        folder_path = os.path.join(os.getcwd(), "bills")
        os.makedirs(folder_path, exist_ok=True)

        bill_filename = os.path.join(folder_path, str(self.bill.get()) + ".txt")
        with open(bill_filename, "w") as f:
            f.write(self.bill_data)

        # Save data to CSV
        sales_file = os.path.join(os.getcwd(), "sales_data.csv")
        sales_data = {
            "Date": [datetime.now().strftime("%Y-%m-%d %H:%M:%S")],
            "Bill No": [self.bill.get()],
            "Customer Name": [self.c_name.get()],
        ...
        if not os.path.isfile(sales_file):
            ...
            df.to_csv(sales_file, mode='a', header=False, index=False)
